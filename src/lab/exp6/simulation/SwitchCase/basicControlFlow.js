...
class Initializer {
	...
	get input() { this.inputValue = $('#textFieldId').val(); return parseInt(this.inputValue); }
...
class Executor {
	...
	initialize(input) {
		...
		let exec3; if (input === 7) exec3 = e31; else if (input >= 1 && input <= 6) exec3 = e32; else exec3 = e33;
	...
	execute(lineNumber) {
		if (lineNumber === 0) {
			...
			getOutputter().str = getOutputter().week[this.day];
...
class Outputter {
	...
	set day(day) {
		this.mDay = day;
		console.assert(day >= 0 && day <= 7, 'Day Index not in Range for Outputter object', this);
